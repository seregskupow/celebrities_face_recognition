...
async function start() {
  ...
  await f.map((item,index)=>{
    let arr1 =new Float32Array(Object.keys(item._descriptors[0]).length);
    let arr2 = new Float32Array(Object.keys(item._descriptors[1]).length);
    for(let i=0;i<Object.keys(item._descriptors[0]).length;i++){
      arr1[i]=item._descriptors[0][i];
    }
    for(let i=0;i<Object.keys(item._descriptors[1]).length;i++){
      arr2[i]=item._descriptors[1][i];
    }
    let d=[arr1,arr2]
    a[index]=new faceapi.LabeledFaceDescriptors(item._label,d)
  })
  console.log(f);
  console.log('rrrr');
  console.log(a)
